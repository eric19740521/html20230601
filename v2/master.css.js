...
const config = {
    ...
    classes: {//2xs 480. xs 600 . sm 768. md 1024. lg 1280. xl 1440.
        bg:{
          primary:'{$primary-rgb:13,110,253;$bg-opacity:1;bg:rgba($(primary-rgb),$(bg-opacity))!;} ',  
          secondary:'{$secondary-rgb:108,117,125;$bg-opacity:1;bg:rgba($(secondary-rgb),$(bg-opacity))!;} ',    
          success:'{$success-rgb:25,135,84;$bg-opacity:1;bg:rgba($(success-rgb),$(bg-opacity))!;}',  
          danger:'{$danger-rgb:220,53,69;$bg-opacity:1;bg:rgba($(danger-rgb),$(bg-opacity))!;}',  
          warning:'{$warning-rgb:255,193,7;$bg-opacity:1;bg:rgba($(warning-rgb),$(bg-opacity))!;}',  
          info:'{$info-rgb:13,202,240;$bg-opacity:1;bg:rgba($(info-rgb),$(bg-opacity))!;}',  
          light:'{$light-rgb:248,249,250;$bg-opacity:1;bg:rgba($(light-rgb),$(bg-opacity))!;}',  
          dark:'{$dark-rgb:33,37,41;$bg-opacity:1;bg:rgba($(dark-rgb),$(bg-opacity))!;}',  
          body:'{$body-bg-rgb:255,255,255;$bg-opacity:1;bg:rgba($(body-bg-rgb),$(bg-opacity))!;}', 
          white:'{$white-rgb:255,255,255;$bg-opacity:1;bg:rgba($(white-rgb),$(bg-opacity))!;}', 
          transparent:'{$bg-opacity:1;bg:transparent!;}', 
        ...
        text:{
          primary:'{$primary-rgb:13,110,253;$text-opacity:1;color:rgba($(primary-rgb),$(text-opacity))!;}',  
          secondary:'{$secondary-rgb:108,117,125;$text-opacity:1;color:rgba($(secondary-rgb),$(text-opacity))!;}',    
          success:'{$success-rgb:25,135,84;$text-opacity:1;color:rgba($(success-rgb),$(text-opacity))!;}',  
          danger:'{$danger-rgb:220,53,69;$text-opacity:1;color:rgba($(danger-rgb),$(text-opacity))!;}',  
          warning:'{$warning-rgb:255,193,7;$text-opacity:1;color:rgba($(warning-rgb),$(text-opacity))!;}',  
          info:'{$info-rgb:13,202,240;$text-opacity:1;color:rgba($(info-rgb),$(text-opacity))!;}',  
          light:'{$light-rgb:248,249,250;$text-opacity:1;color:rgba($(light-rgb),$(text-opacity))!;}',  
          dark:'{$dark-rgb:33,37,41;$text-opacity:1;color:rgba($(dark-rgb),$(text-opacity))!;}',  
          body:'{$body-color-rgb:33,37,41;;$text-opacity:1;color:rgba($(body-color-rgb),$(text-opacity))!;}', 
          muted:'{$text-opacity:1;color:#6c757d!;}',
          white:'{$white-rgb:255,255,255;$text-opacity:1;color:rgba($(white-rgb),$(text-opacity))!;}', 
          'black-50':'color:rgba(0,0,0,.5)!',
          'white-50':'color:rgba(255,255,255,.5)!',
          transparent:'color:transparent!',   
          reset:'{$text-opacity:1;color:inherit!;}', 
       ...
       //下方為CSS Grid
        ...
        font:{
           monospace:'font-family:var(--font-monospace)!important',
        ...
        btn: {
              '': '{display:inline-block;font:16;font-weight:400;text:center;text-decoration:none;p:6|12;b:none;r:4;line-height:1.5;cursor:pointer;user-select:none;} {transform:scale(0.9);}:active {opacity:0.65!;pointer-events:none!;}:disabled {opacity:0.65!;pointer-events:none!;}.disabled {rel;top:-1px;}_.badge',
       ...
       collapse2:'{transform:none;}.show {display:none;transform:translateY(-100%);transition:transform|0.3s|ease-in-out|0s;}:not(.show)', 
        ...
        item:'{pl:0.5rem}+.breadcrumb-item  color:#6c757d:active {float:left;pr:0.5rem;color:#6c757d;content:$(breadcrumb-divider,"/");}+.breadcrumb-item::before', 
        },
